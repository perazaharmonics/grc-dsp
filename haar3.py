...
class haar3(gr.top_block, Qt.QWidget):

    def __init__(self):
        gr.top_block.__init__(self, "haar3", catch_exceptions=True)
        Qt.QWidget.__init__(self)
        self.setWindowTitle("haar3")
        ...
        self.settings = Qt.QSettings("GNU Radio", "haar3")
        ...
        self.h_inv = h_inv = [1 / np.sqrt(2), 1 / np.sqrt(2)]
        self.h = h = [1 / np.sqrt(2), 1 / np.sqrt(2)]
        self.g_inv = g_inv = [1 / np.sqrt(2), -1 / np.sqrt(2)]
        self.g = g = [-1 / np.sqrt(2), 1 / np.sqrt(2)]
        self.frequency = frequency = 50

        ##################################################
        # Blocks
        ##################################################
        self._frequency_range = Range(-samp_rate/2, samp_rate/2, 1, 50, 200)
...
def main(top_block_cls=haar3, options=None):
